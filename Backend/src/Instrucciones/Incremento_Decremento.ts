...
export class Incremento_Decremento extends Instruccion{
  ...
  public ejecutar(entorno: Entorno) {
    ...
    if (simbolo) {
      ...
      let nuevoValor: Resultado = {valor: 0, tipoDato: TipoDato.ENTERO};
      if (simbolo.obtenertipoDato() == TipoDato.ENTERO){
        if (this.signo == "++"){
        ...
        throw new Error(`Tipo ${simbolo.obtenertipoDato()} no es asignable a ${TipoDato.ENTERO}`);
      }
